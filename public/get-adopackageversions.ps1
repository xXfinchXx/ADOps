function get-adopackages {
    ...
    process{
        $final =invoke-restmethod -method Get -uri "https://dev.azure.com/${ADOAccount}/_apis/packaging/Feeds/$($feedid)/packages/$($packageid)?includeAllVersions=true" -ContentType "application/json" -Headers @{Authorization=("Basic {0}" -f $base64AuthInfo)}
    }
    end{
        $final
    }
