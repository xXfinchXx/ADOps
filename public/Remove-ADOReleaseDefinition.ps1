function remove-adoreleasedefinition{
    param(
        [parameter(Mandatory)]$ReleaseDefinitionID
    )
    begin{
        if (!($ADOpat)){
    ...
    process{
        $uri = "https://vsrm.dev.azure.com/$($ADOAccount)/$($ADOprojectName)/_apis/release/definitions/$($ReleaseDefinitionID)?api-version=6.0-preview"
