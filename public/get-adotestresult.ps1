function get-adotestresult {
    Param(
      [Parameter(Mandatory)][string]$runID,
      [Parameter(Mandatory)][string]$testCaseResultId
    )
    begin{
      if (!($ADOpat)){
   ...
   process{
      $uri = "https://dev.azure.com/$($ADOAccount)/$($ADOprojectName)/_apis/test/runs/$($runId)/results/$($testCaseResultId)"
