function remove-adotaskgroup {
    ...
    process{
        $final =invoke-restmethod -method DELETE -uri "https://dev.azure.com/${ADOAccount}/_apis/distributedtask/taskgroups/$($TaskGroupID)" -ContentType "application/json" -Headers @{Authorization=("Basic {0}" -f $base64AuthInfo)}
    }
    end{
        $final
    }
